...
def calculate_payment_exposure(
    *,
    ...
    _require_m14_debt_status(status)
    ...
    if status is DebtStatus.ACTIVE:
        return PaymentExposureUZS(
            max(original_amount.value - posted_total.value, _ZERO_UZS)
        )
...
def open_debt_count_contribution(status: DebtStatus) -> int:
    _require_m14_debt_status(status)
    return int(status in {DebtStatus.PENDING, DebtStatus.ACTIVE})
...
def _require_m14_debt_status(status: DebtStatus) -> None:
    if not isinstance(status, DebtStatus) or status not in M14_PERSISTED_STATUSES:
        raise ValueError("Debt status is outside the M14 persisted subset")
