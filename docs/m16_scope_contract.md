...
GET is historical retrieval only: it creates no M16/domain row, does not lock the target or recompute band, and allows suspended own-Shop reading. Revoked, foreign, guessed, corrupt, or missing results are generic unavailable. Platform-admin status is not a membership bypass.
