...
- Protected unlink/relink re-auth limits: one shared 900-second scope family,
  5 failed attempts per user and 20 per resolved IP; successful verification
  clears only the user bucket.
...
| `POST /auth/telegram/relink-token` | Authenticated current account; only valid while canonically linked. Current-password re-auth is mandatory. | Same one-time reveal response shape as first-link issuance. | CSRF and current password are mandatory; request-owned transaction. Failed verification uses the dedicated user/IP limiter. | Same HTMX-only mutation and non-HTMX `303` fail-safe as link issuance; always `no-store`. | Same privacy contract. Stable lifecycle errors are mapped without internal text. Raw password never enters the Telegram domain service. |
...
| `POST /auth/telegram/unlink` | Authenticated current account plus operation-time current-password re-auth. | Form POST followed by `303` to the status page. | CSRF and current password are mandatory; request-owned transaction. Failed verification uses the dedicated user/IP limiter. | PRG and always `no-store`; HTMX grants no additional authority. | Already-unlinked is a safe no-op with no duplicate event. Raw password never enters the Telegram domain service. |
